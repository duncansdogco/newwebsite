...
const assetVersion = "2026-05-22-8";
...
function clean() {
  for (const entry of fs.readdirSync(ROOT)) {
    if (["assets", "build-site.js", "styles.css", "script.js", "netlify.toml", "gallery-contact.html", "video-contact.html"].includes(entry)) continue;
...
function redirectsAndMeta() {
  ...
  const urls = ["/", ...servicePages.map((p) => `/${p.slug}/`), "/pricing/", "/about-us/", "/areas/", ...areas.map(([slug]) => `/areas/${slug}/`), "/faq/", "/contact/", "/startup-support/", "/careers/", "/privacy-policy/", "/terms-conditions/", "/blog/", ...blogPosts.map(([slug]) => `/blog/${slug}/`)];
