...
const teamMembers = [
  ...
  },
  {
    ...
    bio: "Eleanor runs the PUP programme and has spent four years watching puppies arrive nervous and leave confident. She knows every pup's personality and finds it genuinely rewarding to be part of their development.",
    label: "Her dog",
...
function clean() {
  for (const entry of fs.readdirSync(ROOT)) {
    if ([".git", ".gitignore", ".netlify", "assets", "build-site.js", "styles.css", "script.js", "netlify.toml", "gallery-contact.html", "video-contact.html"].includes(entry)) continue;
...
function layout({ route, title, description, keywords, h1, intro, body, hero = false, heroData = null, image = "assets/woodland.jpg", structured = [] }) {
...
function faqMarkup(faqs) {
  return `<div class="faq-grid">${faqs.map(([q, a]) => `<details><summary>${esc(q)}</summary><p>${esc(a)}</p></details>`).join("")}</div>`;
}
...
function serviceBody(page) {
  ...
  if (page.slug === "puppy-daycare-cobham") {
    const puppyFaqs = [["How old does my puppy need to be?", "We accept puppies from 12 weeks old, provided they have had their primary vaccinations and received veterinary clearance to socialise with other dogs."], ["Are Puppy School sessions separate from adult daycare?", "Yes — completely. Puppy sessions are run separately from our adult woodland daycare groups."], ["How long are the sessions?", "Puppy sessions are shorter than full daycare days, reflecting the exercise and rest needs of young dogs."], ["Do you offer door-to-door collection for Puppy School?", "Yes — collection and drop-off is included for Puppy School just as it is for regular daycare."], ["When does Puppy School transition to full daycare?", "There is no fixed age. It depends on your puppy's development, confidence and readiness."]];
    return `${liveIntro({ kicker: "About puppy school", title: "Separate from the adults. Built for puppies.", paragraphs: ["Puppy School at Duncan's Dog Co. is run as a dedicated pathway — entirely separate from our adult daycare sessions. Puppies need gentler exercise, shorter sessions, more rest, and the right kind of socialisation at the right pace.", "We take puppies from 12 weeks old, once they have had their initial vaccinations. Sessions are puppy-focused in our woodland setting, giving your puppy the benefits of the outdoors without the overwhelm of a large group of adult dogs.", "Everything we do is focused on building confidence, positive associations with new dogs and people, and the calm, happy temperament that makes for a great adult dog."], badge: "Accepting puppies from 12 weeks old", image: gallerySrc(gallery.puppyWoodland), alt: "Puppy in the woodland at Duncan's Dog Co." })}${authenticPhotoRibbon({ kicker: "Why we're different", title: "Real-world confidence, built gently.", text: "Puppies learn through calm handling, careful socialisation, familiar people and positive woodland experiences. We keep introductions gentle, separate from the adult groups, and matched to each puppy's age and confidence.", className: "puppy-ribbon" })}<section class="section live-content puppy-care-section"><div class="section-heading-row reveal"><div><p class="section-kicker">Built into every day</p><h2>Built for where they are.</h2><div class="squiggle-line" aria-hidden="true"></div></div><p>Not a course. Not a programme. Just consistent, expert care woven into each puppy day from the start.</p></div>${richFeatureGrid([["Socialisation", "Positive, consensual interactions with other dogs — learning to read cues, play safely and build confidence with their peers."], ["Training", "Reward-based methods, good manners, recall foundations and calm handling built naturally into the day."], ["Habituation", "Gradual exposure to new sights, sounds and experiences so puppies build confidence without overwhelm."], ["Recall & independence", "Off-lead confidence, safe woodland recall and learning to self-regulate with guidance from the team."], ["Routine & structure", "A consistent rhythm helps puppies understand expectations and settle faster with the same calm approach."], ["Progress & support", "Regular updates, honest guidance and help with questions about behaviour, training and settling at home."]])}</section><section class="section faqs puppy-faq-section"><div class="section-kicker">Common questions</div><h2>Puppy School FAQs.</h2>${faqMarkup(puppyFaqs)}</section>${ctaBand("Ready for Puppy School?", "Book a puppy introduction.", "Spaces in Puppy School are limited. Get in touch to check availability and arrange the right first step.", "/contact/#enquiry-form", "Enquire about Puppy School")}`;
  }
...
const fullFaqSections = [
  ...
  },
  {
    ...
    faqs: [
      ...
      ["How do you handle puppies differently to adult dogs?", "Puppies need more structure, more rest and more careful introductions. We keep puppy groups separate from high-energy adult dogs until they are ready. Our team is experienced in reading early signals of stress or overstimulation and will adjust the day to suit each puppy."]
    ]
  },
  {
    title: "Sleepovers",
  ...
  },
  {
    ...
    faqs: [
      ...
      ["What's your neutering policy?", "We do not believe in one-size-fits-all advice, and we're happy to work with you to find the right time for your dog. For most dogs we suggest neutering or spaying between 12 and 18 months. Larger breeds often benefit from waiting closer to 18 months. If your female dog comes into season, she'll need 10 days at home after her final day of bleeding and her slot is held during this time."],
      ["What if my dog isn't neutered?", "We welcome unneutered dogs provided behaviour is suitable. If your dog shows disruptive or hormonally driven behaviours, we may advise a temporary pause or discuss a neutering timeline. If you choose not to neuter at all, we'll always assess suitability individually."],
      ...
      <div class="form-wrap reveal">
        ...
        <form name="enquiry" method="POST" action="/thank-you/" data-netlify="true" data-netlify-honeypot="bot-field">
          ...
          <div class="form-notes">
            <div class="form-note-card"><strong>Sleepovers:</strong> A successful trial daycare visit is required before any overnight stay.</div>
...
function redirectsAndMeta() {
  ...
  const urls = ["/", ...servicePages.map((p) => `/${p.slug}/`), "/pricing/", "/about-us/", "/areas/", ...areas.map(([slug]) => `/areas/${slug}/`), "/faq/", "/contact/", "/thank-you/", "/startup-support/", "/careers/", "/privacy-policy/", "/terms-conditions/", "/blog/", ...blogPosts.map(([slug]) => `/blog/${slug}/`)];
